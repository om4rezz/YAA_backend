    ...
    for file, index in files
      ...
      req = request.post options, (error, response) =>
        ...
        if response.statusCode is 200
          file = response.request.headers.YAAFilename
          number = response.request.headers.YAARequest
      ...
      if file.indexOf('min/') > -1 and file.indexOf('map') is -1
        form.append 'url', "https://app.YAA.com#{file.substr file.indexOf '/'}?#{grunt.option 'version'}"
      else
        form.append 'url', "https://app.YAA.com#{file.substr file.indexOf '/'}"
