...
class z.telemetry.calling.CallTelemetry
  ...
  trace_request: (conversation_id, jqXHR) ->
    timestamp_incoming = jqXHR.YAA.responded.getTime()
    timestamp_outgoing = jqXHR.YAA.requested.getTime()

    request_duration = timestamp_incoming - timestamp_outgoing
    request_type = "#{jqXHR.YAA.original_request_options.type} #{jqXHR.YAA.original_request_options.api_endpoint}"
    ...
    trace_payload = {}
    if jqXHR.YAA.original_request_options.data
      trace_payload = JSON.parse z.util.types.convert_array_buffer_to_string jqXHR.YAA.original_request_options.data
    ...
    @traces[conversation_id].push {
      ...
      transport: 'REST'
      request:
        method: jqXHR.YAA.original_request_options.type
        url: jqXHR.YAA.original_request_options.url
