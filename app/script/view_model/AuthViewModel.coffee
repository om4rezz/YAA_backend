...
class z.ViewModel.AuthViewModel
  ...
  _init_base: ->
    ...
    $("[id^='YAA-login'], [id^='YAA-mail'], [id^='YAA-register'], [id^='YAA-phone-code']").prevent_prefill()
  ...
  clicked_on_login: =>
    @_set_hash z.auth.AuthView.MODE.ACCOUNT_LOGIN
    $('#YAA-login-phone').focus_field() if @visible_method() is z.auth.AuthView.MODE.ACCOUNT_PHONE
  ...
  clicked_on_YAA_link: ->
    ...
    (z.util.safe_window_open z.localization.Localizer.get_text z.string.url_YAA)?.focus()
  ...
  keydown_phone_code: (view_model, event) =>
    ...
    switch event.keyCode
      when z.util.KEYCODE.ARROW_LEFT, z.util.KEYCODE.ARROW_UP
        ...
        $("#YAA-phone-code-digit-#{Math.max 1, focus_digit}").focus()
      when z.util.KEYCODE.ARROW_DOWN, z.util.KEYCODE.ARROW_RIGHT
        focus_digit = target_digit + 1
        $("#YAA-phone-code-digit-#{Math.min 6, focus_digit}").focus()
  ...
  show_account_register: (focus = 'YAA-register-name') ->
    switch_params =
  ...
  show_verify_code: =>
    ...
    switch_params =
      ...
      focus: 'YAA-phone-code-digit-1'
    @switch_ui switch_params
    $('#YAA-phone-code-digit-1').focus()

  show_verify_mail: ->
    switch_params =
      section: z.auth.AuthView.SECTION.VERIFY
      mode: z.auth.AuthView.TYPE.EMAIL
      focus: 'YAA-mail-email'
    @switch_ui switch_params
  ...
  _track_app_launch: ->
    mechanism = 'direct'
    if document.referrer.startsWith 'https://YAA.com/verify/'
      mechanism = 'email_verify'
    else if document.referrer.startsWith 'https://YAA.com/forgot/'
...
$ ->
  if $('.auth-page').length isnt 0
    YAA.auth.view = new z.ViewModel.AuthViewModel 'auth-page', YAA.auth
