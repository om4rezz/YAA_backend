...
class z.ViewModel.DebugViewModel
  ...
  inject_audio_to_participants: ->
    YAA.app.repository.call_center.count_flows @conversation().id

  download_call_trace: ->
    file_name = "#{YAA.app.repository.user.self().name()} (#{YAA.app.repository.user.self().id}).js"
    text = JSON.stringify YAA.app.repository.call_center.log_trace()
  ...
  log_call_to_console: ->
    YAA.app.repository.call_center.log_call()

  log_call_banner_state_to_console: ->
    YAA.app.view.content.call_controls.log_state()

  log_session_ids_to_console: ->
    YAA.app.repository.call_center.log_calls()
