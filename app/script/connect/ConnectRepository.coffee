...
class z.connect.ConnectRepository
  ...
  @return [Promise] Promise that resolves with the user's Google contacts that match on YAA
  ###
  get_google_contacts: ->
  ...
  @return [Promise] Promise that resolves with the user's address book contacts that match on YAA
  ###
  get_osx_contacts: ->
    return new Promise (resolve, reject) =>
