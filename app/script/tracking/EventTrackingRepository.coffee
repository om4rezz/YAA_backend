...
class z.tracking.EventTrackingRepository
  ...
  ###
  @return [Boolean] true when "improve_YAA" is set to "true".
  ###
  _has_permission: ->
    return false if @user_properties is undefined
    return false if @tracking_id is undefined
    return @user_properties.settings.privacy.improve_YAA
  ...
  _enable_error_reporting: ->
    ...
    ###
    Adding a version to the Raygun reports to identify which version of the YAA ran into the issue.
