...
APP_ENV =
  LOCALHOST: 'localhost'
  PRODUCTION: 'app.YAA.com'
  PROD_NEXT: 'YAA-webapp-prod-next.YAA.com'
  TACO: 'taco.YAA.com'
  VIRTUAL_HOST: 'YAA.ms' # The domain "YAA.ms" is our virtual host for testing contact uploads
...
z.util.Environment = do ->
  ...
  app_version = ->
    if $("[property='YAA:version']").attr('version')?
      version = $("[property='YAA:version']").attr('version').trim().split '-'
